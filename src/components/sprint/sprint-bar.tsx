...
export function SprintBar({ projectId, projectKey }: SprintBarProps) {
  ...
  const totalPoints =
    activeSprint?.tickets.reduce((sum, t) => sum + (t.storyPoints ?? 0), 0) ?? 0;
      ...
      <>
        <div className="flex items-center gap-3 border-b bg-muted/30 px-6 py-2">
          ...
          <div className="flex items-center gap-2 ml-auto">
            ...
            {/* Burndown button */}
            <Button variant="ghost" size="sm" className="h-7 text-xs" onClick={() => setBurndownOpen(true)}>
  ...
  // No active sprint — show planning sprint or create button
  return (
    <>
      <div className="flex items-center gap-3 border-b bg-muted/30 px-6 py-2">
          ...
          {planningSprints.length > 0 && (
            <>
              <span className="text-xs text-muted-foreground">
                {planningSprints[0].name} ({planningSprints[0]._count.tickets} tickets)
              </span>
              <Button
                size="sm"
                className="h-7 text-xs"
