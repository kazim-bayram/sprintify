...
const ICONS: Record<string, React.ElementType> = {
  ...
  LABEL_ADDED: Tag,
  LABEL_REMOVED: Tag,
  STORY_POINTS_CHANGED: Hash,
  ATTACHMENT_ADDED: Paperclip,
  ATTACHMENT_REMOVED: Paperclip,
  TICKET_ARCHIVED: Archive,
  TICKET_RESTORED: RotateCcw,
};

function describeActivity(type: string, data: Record<string, unknown> | null): string {
  switch (type) {
    case "STATUS_CHANGE":
      return `moved from ${data?.from ?? "?"} to ${data?.to ?? "?"}`;
    case "PRIORITY_CHANGE":
      return `changed priority from ${data?.from ?? "None"} to ${data?.to ?? "None"}`;
    case "ASSIGNED":
    ...
    case "STORY_POINTS_CHANGED":
      return `changed story points from ${data?.from ?? "none"} to ${data?.to ?? "none"}`;
    case "ATTACHMENT_ADDED":
      return `attached "${data?.fileName ?? "a file"}"`;
    case "ATTACHMENT_REMOVED":
      return `removed "${data?.fileName ?? "a file"}"`;
...
export function ActivityFeed({ activities }: { activities: ActivityItem[] }) {
  if (activities.length === 0) {
    return <p className="text-center text-xs text-muted-foreground py-4">No activity yet</p>;
  }
