...
interface LabelSelectProps {
  ticketId: string;
  currentLabels: TicketLabel[];
}

export function LabelSelect({ ticketId, currentLabels }: LabelSelectProps) {
  const utils = trpc.useUtils();
  const { data: allLabels } = trpc.label.list.useQuery();

  const addLabel = trpc.label.addToTicket.useMutation({
    onSuccess: () => {
  ...
  const removeLabel = trpc.label.removeFromTicket.useMutation({
    onSuccess: () => {
      utils.ticket.getById.invalidate({ id: ticketId });
    },
    onError: (e) => toast.error(e.message),
          ...
          <PopoverContent className="w-56 p-2" align="end">
            ...
            {allLabels && allLabels.length > 0 ? (
              ...
              <p className="text-xs text-muted-foreground">
                No labels. Create them in Settings &gt; Labels.
              </p>
            )}
          </PopoverContent>
        </Popover>
        ...
        <div className="flex flex-wrap gap-1">
          {currentLabels.map((tl) => (
            <Badge
              key={tl.label.id}
              variant="secondary"
              style={{ borderColor: tl.label.color, borderLeftWidth: 3 }}
              className="text-[10px] gap-1 pr-1"
            >
              {tl.label.name}
              <button
                onClick={() => removeLabel.mutate({ ticketId, labelId: tl.label.id })}
