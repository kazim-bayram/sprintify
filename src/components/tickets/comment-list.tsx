...
export function CommentList({ ticketId, initialComments }: CommentListProps) {
  ...
  const createMutation = trpc.comment.create.useMutation({
    onSuccess: () => {
      setNewComment("");
      utils.comment.list.invalidate({ ticketId });
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    createMutation.mutate({ ticketId, body: newComment.trim() });
  }
            ...
            <div key={c.id} className="group flex gap-3">
              <Avatar className="h-7 w-7 shrink-0">
                ...
                <AvatarFallback className="text-[10px]">
                  {c.user.name?.charAt(0) ?? c.user.email.charAt(0)}
                </AvatarFallback>
              </Avatar>
                ...
                {editingId === c.id ? (
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                      <Button
                        size="sm"
                        onClick={() => updateMutation.mutate({ id: c.id, body: editBody })}
