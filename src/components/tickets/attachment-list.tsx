...
export function AttachmentList({ ticketId, attachments }: AttachmentListProps) {
  ...
  function handleUploadClick() {
    // Supabase Storage integration — will be wired in when bucket is configured
  ...
  return (
    <div className="space-y-3">
      ...
      {attachments.length > 0 ? (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[10px] text-muted-foreground">
                  {formatFileSize(a.size)} · {a.user.name ?? "Unknown"}
                </p>
              </div>
        ...
        <p className="text-center text-xs text-muted-foreground py-4">No files attached</p>
      )}
    </div>
  );
}
